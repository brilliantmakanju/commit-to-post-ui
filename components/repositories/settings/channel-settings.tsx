			...
			<CardContent className="space-y-6">
					...
					{Object.entries(socialConnections).map(([platform, connected]) => {
						...
						return (
							...
							>
								...
								<Switch checked={isConnected} disabled={isSlackOrDiscord} />
							</div>
						);
					})}
				</div>
				...
				<div className="flex items-center justify-between">
					...
					<Switch
						...
						onCheckedChange={checked => onChange("hashtag_automation", checked)}
					/>
				</div>
				...
				{localSettings.hashtag_automation && (
					<div className="space-y-2">
						...
						/>
						<p className="text-xs text-zinc-400">
							Default hashtags to include in posts (comma-separated)
						</p>
					</div>
				)}
			</CardContent>
